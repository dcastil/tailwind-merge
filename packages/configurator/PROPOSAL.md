...
- **P4 — future.** Used-class scanning ("minification" of the config), bundler plugins with virtual modules + HMR (Vite plugin designed and prototyped 2026-08-09 in `packages/vite/`, see §11), a Tailwind version matrix in CI (probe + generation suite against tailwindcss 4.0 → latest to catch `__unstable__` drift early), and — with adoption data in hand — reopening the upstream conversation with the Tailwind team.
...
1. **Verified, with an amendment**: `resolveId` interception of the real subpath works in dev and build — but only with `enforce: 'pre'` (§11.2). The dev cold-start scan with `optimizeDeps.exclude` hasn't been exercised against a browser-driven client pipeline yet, only SSR and build.
2. **Closed by design** (2026-08-09, second review round): the virtual module no longer imports `'tailwind-merge'` at all — it imports the plugin package's own re-export subpath (§11.3), so every resolution step is ordinary node_modules resolution and there is no redirect left to verify. The fixtures' `node_modules` symlink exercises exactly this path. A packed-tarball install test remains worthwhile before publishing, but as a general release gate (à la `test:exports`), not for this mechanism specifically.
...
4. **Open**: dependency-set parity with `@tailwindcss/vite`'s watch list — blocked on installing `@tailwindcss/vite` (published 2026-08-07, clears the 3-day `minimumReleaseAge` on 2026-08-10), which is also what a coexistence smoke test needs.
...
6. **Verified at source level** (2026-08-09): the `consumer-types` fixture is a standalone consumer project (own tsconfig, no plugin-related configuration beyond the workspace's source-instead-of-dist paths substitution for tailwind-merge) whose `tsc` run in the suite proves the subpath's exports, types, and `extendTailwindMerge` generics resolve through ordinary package resolution. The published-shape variant (packed tarball, `.d.ts` instead of source) joins the release-gate script alongside `test:exports` once the plugin has a build step.
