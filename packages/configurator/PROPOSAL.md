...
2. **Open**: the importer-scoped redirect of `'tailwind-merge'` inside the virtual module is implemented but shadowed in tests by the workspace alias that maps tailwind-merge to library source. Verifying it needs a fixture with a real pnpm install of the packed plugin.
...
6. **Open**: zero-tsconfig types for a consumer project (`tsc --noEmit` against a fixture that imports the subpath) — the fixture app is currently excluded from the package tsconfig as consumer-app input.
...
8. **Open**: `hotUpdate` for out-of-root dependencies after `server.watcher.add` (the monorepo shared-theme case) — implemented, not yet fixture-tested.
