...
- When probed arbitrary slash modifiers change a functional root's effects, that root is left unmerged entirely, including its bare values. The runtime's postfix fallback prevents safely ignoring only the unknown modified classes. This conservative behavior applies with both encodings and pruning; redundant classes can remain.
