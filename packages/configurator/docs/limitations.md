...
- Opposite custom functional roots (`shift-*` and `-shift-*`) share one runtime lookup path. They remain unmerged unless both have uniform, mutually covering effects. Redundant classes of the same sign can remain when the two roots cannot safely share a group.
