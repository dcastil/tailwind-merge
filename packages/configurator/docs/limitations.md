...
- Custom conflict inference uses compiled declarations, known CSS shorthand relationships, and representative built-in classes. Partial overlaps, unknown shorthand relationships, conditional styles, and pseudo-element effects are handled conservatively; automatic inference does not establish every possible relationship. Built-in-to-custom override inference is not general; aliases cover the cases that can join a built-in group safely.
- A custom functional root with different effects across its names or probed value kinds is split into separate groups. Such roots enumerate names in both encodings and omit broad arbitrary-value matchers; some valid overlapping arbitrary classes therefore remain unmerged. Probes cover the supported arbitrary data types, but representative samples do not validate every possible value or explicit type label.
- When suggested named modifiers or probed bare numeric/arbitrary slash modifiers add effects that the available full-class matchers cannot represent, that functional root is left unmerged entirely, including its bare values. The runtime's postfix fallback prevents safely ignoring only the unknown modified classes. This conservative behavior applies with both encodings and pruning; redundant classes can remain.
- Opposite custom functional roots (`shift-*` and `-shift-*`) share one runtime lookup path. They remain unmerged unless both have uniform, mutually covering effects. Opposite static names (`pull` and `-pull`) also remain unmerged when their effects do not cover each other. Incompatible positive custom static names at the same paths preserve the functional roots and their overlapping static names, including potential aliases. Redundant classes of the same sign can remain when these claims cannot safely share a group.
- A custom `@utility` that extends a built-in functional root (`@utility text-*` adding a text-indent value) is not planned, because a registry diff cannot tell how the extension changes the built-in's behavior: its values are classified only as far as the built-in's own matchers accept them, so a `text-4` from that extension merges with nothing. The root is reported in `unassignedClasses` as `text-*`. Choose a root of its own for such a utility.
