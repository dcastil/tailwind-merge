...
- Opposite custom functional roots (`shift-*` and `-shift-*`) share one runtime lookup path. They remain unmerged unless both have uniform, mutually covering effects. Incompatible positive custom static names at the same paths also preserve the functional roots and their overlapping static names, including potential aliases. Redundant classes of the same sign can remain when these claims cannot safely share a group.
