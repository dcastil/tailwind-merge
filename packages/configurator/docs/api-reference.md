...
An optional `integration` supplies the same `TailwindIntegration` hooks as `generate`: async `resolveCss(id, base)` and `resolveJs(id, base)` return an absolute path to a stylesheet or module, respectively. Both hooks are optional; pass only `onDependency` to observe ordinary filesystem/package loading. Either resolver can return `false`/`undefined` to defer to Tailwind's normal resolution, including explicit `.pcss` or extensionless stylesheet imports. Either hook can also return an alias-expanded request before its extension is known; the appropriate stylesheet or module resolver completes the lookup and reports missing targets under the expanded path.
