...
The optional `onDependency(file)` callback observes resolved dependencies, including those discovered before generation fails. Failed stylesheet and JavaScript config/plugin resolution also report attempted missing paths so an integration can watch for their creation; callback paths are not guaranteed to exist. Configs/plugins are reported before execution, and fresh resolution allows retrying immediately after a missing target is created. Pass the same hooks to scanning and generation so both see the same imports, aliases, and source directives.
