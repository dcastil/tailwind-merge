...
Overlapping names such as `demo-*` and `demo-child-*` are classified from their own compiled classes. Even in compact mode, a parent with registered descendants uses compiled names and supported value types instead of a catch-all matcher. This keeps a preserved child such as `thing-width-2/[3px]` independent of an opacity utility such as `thing-0.5`. A bare utility and its functional form share a group only when their effects fully cover each other; a separate static `demo-child` can therefore keep different conflict behavior from `demo-child-*`.
...
Custom-property declarations also participate in coverage, including names such as `--_size`, `--2xl`, Unicode identifiers, and escaped punctuation. Their values can contain balanced blocks such as `--state: {ready}` or nested brackets and semicolons; those tokens remain part of the declaration. A color utility that sets custom-property state survives a later text color so another class or JavaScript can still read the variable.
