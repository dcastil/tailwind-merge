...
- **While the version is 0.x, minor releases can contain breaking changes** — to plugin options, the runtime module's exports, generated merge behavior, or supported Tailwind/Next.js ranges. Patch releases contain only fixes.
...
- The stable surface the versioning promises apply to is the **plugin's options** and the **`@tailwind-merge/next/runtime` exports**. Everything else — the shape of the generated module, the loader module, the config-generation machinery — is internal and may change in any release.
