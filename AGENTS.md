...
- Tests: `tests/`
- Human docs: `docs/`
- Sibling workspace packages (not published, not part of the library build): `packages/` — the configurator exploration (research and prototype for generating tailwind-merge configs from Tailwind v4 CSS) in `packages/configurator/`, and the Vite plugin wrapping it in `packages/vite/`. The root `tailwind-merge` package may migrate into `packages/` later.
