...
- Unreleased workspace packages: `packages/configurator/` generates tailwind-merge configs from Tailwind v4 CSS; `packages/plugin-core/` adds what every bundler plugin needs on top (CSS root discovery, runtime-module generation, the generation session); `packages/vite/` wraps both for Vite with automatic generation and usage pruning. The configurator and the Vite plugin have a maintained `README.md` and user documentation in `docs/`; the plugin core is internal and inlined into the plugin.
- Repo root: workspace orchestration only — `pnpm-workspace.yaml` (workspace list + shared tool-version catalog), `eslint.config.base.mjs` (shared lint base every scope spreads), root `eslint.config.mjs` (lints root files and `.github/`), root `vitest.config.mts` (aggregates the packages' Vitest projects plus the local GitHub actions' tests), repo-wide release tooling in `scripts/` (`update-pinned-links.mjs`, `pin-readme-links.mjs`, `stamp-dev-version.mjs`), and the repo-level `README.md`.
...
- No task runner (turborepo or similar) and no affected-only CI on purpose: at the current package count the full suite is fast, and running everything keeps cross-package effects (a tailwind-merge or plugin-core change must exercise the Vite plugin tests) trivially correct. Revisit when the package count grows or build times make caching pay off.
...
4. The library's `README.md` and the generated section of the repo-level `README.md` are version-generated from the library's `docs/README.md`; do not regenerate them during normal development changes. The configurator and Vite READMEs are maintained landing pages, not generated files; edit them alongside their own `docs/` pages.
...
- Configurator, plugin core, or the Vite plugin: run `pnpm --filter @tailwind-merge/configurator test` / `pnpm --filter @tailwind-merge/plugin-core test` / `pnpm --filter @tailwind-merge/vite test` and the matching `test:types` script. A core or configurator change must run the Vite plugin suite as well.
