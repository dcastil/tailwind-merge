...
- Static custom aliases must match exactly one built-in signature and cover each other's effects. A base color plus a hover/dark color cannot alias an unconditional color, even though their property-name sets match. Other roots self-conflict; a later custom class may remove another group only when it fully covers all its declarations. Built-in-to-custom edges are not inferred generally. Conditional/pseudo-element declarations may only be shared identical scaffolding, including their full enclosing rule paths, and custom-property state must be covered as well. `DeclarationEntry.scope` preserves those headers relative to the utility selector; compare with `sameDeclarationScope`, not the `conditional` flag alone. Conditions are compared conservatively as text, without logical equivalence inference.
...
- Custom functional classes are indexed once by their longest functional root, with exact static names excluded. Exact encoding and conflict exemplars share that ownership rule: using the first `startsWith(root + '-')` suggestion can mistake `demo-child` or `demo-child-*` for `demo-*` and infer unrelated conflicts. Keep each group's exemplar beside its members instead of reconstructing roots from group IDs. The overlapping-root regressions exercise both encodings, import prefixes, emitted round trips, and pruning.
