...
npm cannot attach a trusted publisher to a package name that has never been published, so a plugin's first publish is a one-time local dev publish with a token — the only step of the whole flow that uses one — after which the trusted publisher covers all later dev and stable publishes. A new plugin stays out of the dev jobs until it is bootstrapped, because a publish step without a trusted publisher fails on every `main` push; both current plugins went through this. The bootstrap, on a clean checkout of the pushed `main` commit whose library dev release has finished publishing — shown for `@tailwind-merge/next`, the Vite plugin's was the same with `packages/vite`:
...
Publish without `--provenance`: provenance needs a CI OIDC token and fails locally. Then configure the trusted publisher (repository `dcastil/tailwind-merge`, workflow `npm-publish.yml`, no environment) on the package's npm access page or with `npx npm@latest trust github @tailwind-merge/next --file npm-publish.yml --repo dcastil/tailwind-merge --allow-publish` (`npm trust` needs npm 11.15 or newer). Then add the package to both dev jobs the way the plugins are wired: the build and `test:exports` filters, an artifact upload and download for its `dist`, the filtered install, a stamp line, and a publish step after the library's. With the trusted publishers in place, the publish workflow's dev jobs publish all three packages on every `main` push: the build job builds them and runs every `test:exports` gate, the OIDC job stamps all manifests with the script and publishes the library before the plugins, so the plugins' exact pins always resolve. A bootstrap publish sets `latest` as well as `dev` on the plugin, which npm does for a package's first version regardless of `--tag`; the first stable release moves `latest` to a real version.
