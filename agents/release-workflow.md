...
npm cannot attach a trusted publisher to a package name that has never been published, so a plugin's first publish is a one-time local dev publish with a token — the only step of the whole flow that uses one — after which the trusted publisher covers all later dev and stable publishes. Until a plugin's trusted publisher exists, the dev publish job's `pnpm publish` for that package fails on every `main` push (the library's and the other plugin's publishes in the same job run before it, so they are unaffected), which is the expected state between merging a new plugin and bootstrapping it. The bootstrap, on a clean checkout of the pushed `main` commit whose library dev release has finished publishing — shown for `@tailwind-merge/next`, the Vite plugin's was the same with `packages/vite`:
