...
- Covers:
    ...
    - Changelog updates in `packages/tailwind-merge/docs/changelog/*-changelog.md` (each plugin gets its own changelog location with its first release).
...
`.github/workflows/draft-release.yml` maintains one draft per package via release-drafter with per-package configs (`config-name`): `.github/release-drafter-tailwind-merge.yml`, `.github/release-drafter-vite.yml`, and `.github/release-drafter-next.yml`. Each config scopes itself with `tag-prefix` (previous-release lookup and version resolution consider only that package's tags) and `include-paths` (only PRs touching the package or its bundled source appear in its draft). Consequences to know:

- A PR touching several packages appears in each of their drafts — correct, since each release ships it.
- Both plugin scopes include `packages/plugin-core` and `packages/configurator` as well as their own directory: fixes in the shared code ship in both plugins' bundles and belong in both changelogs. The library remains a separate dependency with its own release scope.
...
- First namespaced release per package: `tag-prefix` matches no existing release, and the pinned release-drafter (v7) then drafts from nothing rather than from full history — its previous-release lookup keeps only releases whose tag starts with the prefix, and without one it collects no pull requests at all. The draft therefore proposes version `0.1.0` and its body is the `no-changes-template` ("No changes") plus a warning banner, while the pre-existing unprefixed draft (`v3.6.1` for tailwind-merge) keeps the real, categorized pull request list and is never updated or removed by the drafter again. When publishing: copy the categorized list from that orphaned draft into the new one, then delete the orphaned draft; set the tag name and title by hand (for example `tailwind-merge@3.7.0` following `v3.6.0`); and write the Full Changelog compare line manually (`v3.6.0...tailwind-merge@3.7.0`). For the plugins the proposed `0.1.0` happens to be right, but their pull request lists are equally empty and must be written from the PRs touching the package, `packages/plugin-core`, and `packages/configurator`.
- Tag-pinned file links repo-wide (the AGENTS.md link policy) are maintained automatically: each publishable package's `version` lifecycle runs `scripts/update-pinned-links.mjs`, which rewrites every link pinned to that package's newest existing tag to the tag being released (logging each rewrite for release-time review), verifies each target path against the working tree (failing the version step loudly on unresolvable paths, touching nothing), remaps pre-monorepo root paths into the package directory, and leaves links pinned to older tags alone as deliberately historical. Historical links that sit at the newest tag anyway are covered by two escape hatches: changelog directories are never scanned (their entries — including the one for the latest release — always describe a specific past release), and a link carrying the `twm-historical` query parameter stays untouched and exempt from path validation. First release of a package is a no-op for it. Concretely: the tailwind-merge 3.7.0 release re-pinned the vite docs' `v3.6.0` links to `tailwind-merge@3.7.0` with `packages/tailwind-merge/` paths on its own. Relative README links are the complementary case, handled by `scripts/pin-readme-links.mjs`: the plugin's `version` lifecycle runs it after the re-pin to turn the README's `./docs/...` and `../../agents/...` links into absolute links pinned to the tag being released (staging the README for the version commit), because npm cannot resolve relative links, while the repo keeps them relative between releases; the dev-release stamp imports the same helper and pins to the commit instead. Links it has pinned to a tag are absolute from then on and fall under `update-pinned-links.mjs` at later releases. The library needs neither: its README is generated from `docs/README.md` with absolute links by `scripts/update-readme.mjs`.
- The autolabeler section lives only in the tailwind-merge config; labels are repo-wide.

## Publishing

`.github/workflows/npm-publish.yml` routes `release.published` events by tag prefix: `tailwind-merge@*` and legacy `v*` build and publish `packages/tailwind-merge`, `@tailwind-merge/vite@*` builds and publishes `packages/vite`, `@tailwind-merge/next@*` builds and publishes `packages/next`, and unknown prefixes fail the run. The build job runs repo-wide lint and tests, builds the library alongside the released package (a plugin's `test:exports` verifies its packed tarball against the library's published shape), runs the released package's `test:exports`, and for plugin releases `test:library-release`; publishing happens from the package directory in an isolated OIDC job via `pnpm publish` — after a scripts-disabled install of the released package's dependencies, which pnpm needs to rewrite `workspace:` specifiers (the plugins' tailwind-merge dependency), and with pnpm applying `publishConfig` overrides at pack time (the vite package's dist-exports swap) and handling npm trusted publishing natively, `--provenance` passed explicitly because pnpm does not read `publishConfig.provenance` (mechanics in `agents/tailwind-merge-internals.md`). Dev releases on `main` pushes cover the library and the Vite plugin; the Next.js plugin joins them after its manual first publish (see [Dev releases](#dev-releases)).
...
1. Done with `tailwind-merge@3.7.0`, which ships the generator's required `themeKey`, classification/parser exports, and matching configuration semantics; the plugin's `workspace:^` dependency now packs as `tailwind-merge: ^3.7.0` (the caret range keeps later library patches installable without a plugin release). The release build job guards this for every plugin release: `pnpm --filter @tailwind-merge/vite test:library-release` (the shared `scripts/check-library-release.mjs`, given the source trees to scan with `--sources`) resolves the dependency range's lowest version, downloads that release from npm, and fails unless it ships every value and type the bundled plugin core and configurator import from the unstable entry point — a packed check against the workspace library cannot establish that. When it fails, release the library first and bump the plugin's dependency to it; `--library-version` checks an arbitrary release by hand.
...
The Next.js plugin follows the Vite plugin's model exactly: initial release `0.1.0`, the plugin core and the configurator inlined, the library as a regular dependency packed from `workspace:^`, `test:library-release` in the release build job (its `--sources` list covers the configurator, the plugin core, and the plugin), and a trusted publisher set up through the dev-release bootstrap below. The Next.js-specific facts a release must respect are in the [Next.js guide](./next-plugin.md#release-boundaries). Release the library update the plugin's imports need before the plugin, as for the Vite plugin.

### Dev releases

`scripts/stamp-dev-version.mjs` prepares a package for a dev publish; run it from the package directory. It stamps `<version>-dev.<sha>` (`GITHUB_SHA` in CI, `HEAD` locally), pins `workspace:` runtime dependencies to the exact same-SHA dev version of that workspace package (the plugins' bundled configurator imports the library's unstable entry point, so a registry release could lack the internals the same commit changed), and rewrites relative README links to commit-pinned absolute links so the npm page resolves them (through the shared `scripts/pin-readme-links.mjs`, see Release drafting above). `--check-registry` verifies that every pinned dependency version is already on npm; a local publish needs that, CI does not because it publishes the library first in the same job. The script dirties the working tree by design; restore the manifest and README from git after publishing locally.

npm cannot attach a trusted publisher to a package name that has never been published, so a plugin's first publish is a one-time local dev publish with a token — the only step of the whole flow that uses one — after which the trusted publisher covers all later dev and stable publishes. A new plugin therefore stays out of the dev jobs until it is bootstrapped: a publish step without a trusted publisher would fail on every `main` push. `@tailwind-merge/next` is in that state; the bootstrap, on a clean checkout of the pushed `main` commit whose library dev release has finished publishing — the Vite plugin's was the same with `packages/vite`:

```bash
pnpm install --frozen-lockfile
pnpm --filter tailwind-merge --filter @tailwind-merge/next build
pnpm --filter @tailwind-merge/next test:exports
cd packages/next
...
Publish without `--provenance`: provenance needs a CI OIDC token and fails locally. Then configure the trusted publisher (repository `dcastil/tailwind-merge`, workflow `npm-publish.yml`, no environment) on the package's npm access page or with `npx npm@latest trust github @tailwind-merge/next --file npm-publish.yml --repo dcastil/tailwind-merge --allow-publish` (`npm trust` needs npm 11.15 or newer). With a plugin's trusted publisher in place, add the package to both dev jobs the way the Vite plugin is wired: the build and `test:exports` filters, an artifact upload and download for its `dist`, the filtered install, a stamp line, and a publish step after the Vite one. The dev jobs then publish the library and the plugins on every `main` push — the build job builds them and runs every `test:exports` gate, the OIDC job stamps all manifests with the script and publishes the library before the plugins, so the plugins' exact pins always resolve. A bootstrap publish sets `latest` as well as `dev` on the plugin, which npm does for a package's first version regardless of `--tag`; the first stable release moves `latest` to a real version.
