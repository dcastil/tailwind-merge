...
- The `version` script also runs the repo-wide `scripts/update-pinned-links.mjs` (repo root, dependency-free Node; the vite package additionally runs `scripts/pin-readme-links.mjs`, see the release guide), which re-pins every link pinned to the package's newest existing tag onto the tag being released, verifying targets against the working tree and failing loudly — without touching anything — when a linked path no longer resolves. Older-tag links are deliberately historical and stay untouched, changelog directories are excluded from scanning entirely, and the `twm-historical` query parameter on a link shields it individually for the rare historical link elsewhere.
...
- `.github/workflows/test.yml` runs `pnpm lint`, `pnpm test:types`, `pnpm test`, `pnpm --filter tailwind-merge build`, `pnpm --filter tailwind-merge test:exports`, then each plugin's `build` and `test:exports` (`@tailwind-merge/vite`, then `@tailwind-merge/next`) — the plugin steps after the library build because the packed-tarball gates resolve tailwind-merge through its published dist and types. The Next.js package's Vitest project spawns real Next.js processes and builds the library itself when its dist is missing (its global setup), so `pnpm test` stays independent of the build steps' order.
...
- `.github/workflows/npm-publish.yml`:
    - publishes `dev` tag builds of tailwind-merge and both plugins on `main` pushes: one build job builds all three (library first, topologically) and runs every `test:exports` gate, one OIDC job stamps the manifests with `scripts/stamp-dev-version.mjs` (dev version plus the plugins' exact pins on the library's same-commit dev version) and publishes the library before the plugins. The plugin core and the configurator have no dev release; they ship inlined in the plugin bundles. A new package joins these jobs only after its npm trusted publisher exists, since a publish step without one fails on every push (bootstrap in the release guide),
    - publishes production on release events, routing by the namespaced tag prefix (`tailwind-merge@*` and legacy `v*` map to `packages/tailwind-merge`, `@tailwind-merge/vite@*` to `packages/vite`, `@tailwind-merge/next@*` to `packages/next`, anything else fails),
    - the release build job builds the library alongside the released package (`pnpm --filter tailwind-merge --filter "$PACKAGE_NAME" build` — a plugin's test:exports needs the library's published shape; duplicate filters collapse for tailwind-merge releases, and pnpm's topological order builds the library before a plugin) and runs the released package's `test:exports` unconditionally, plus `test:library-release` for plugin releases (registry check that the packed tailwind-merge range's lowest version ships the unstable-entry-point imports; see the Vite and Next.js guides),
    ...
    - grants `id-token: write` only to minimal publish jobs that download the verified `dist` artifact and run `pnpm publish --ignore-scripts`. The release publish job first runs `pnpm install --frozen-lockfile --ignore-scripts --filter "$PACKAGE_NAME"` because pnpm rewrites `workspace:` specifiers at publish time by reading the linked package's manifest from `node_modules` — without it, publishing a plugin fails with `ERR_PNPM_CANNOT_RESOLVE_WORKSPACE_PROTOCOL`. The filter installs only the released package's dependencies, and `--ignore-scripts` keeps dependency code from running in the job. The dev publish job runs the same filtered install for both plugins because their devDependencies keep the inlined plugin core's and configurator's `workspace:*` specifiers; tailwind-merge alone would need no install, since its specifiers are all `catalog:` and resolve from `pnpm-workspace.yaml`,
