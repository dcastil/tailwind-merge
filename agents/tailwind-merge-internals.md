...
- `.github/workflows/npm-publish.yml`:
    - publishes `dev` tag builds of tailwind-merge and both plugins on `main` pushes: one build job builds all three (library first, topologically) and runs every `test:exports` gate, one OIDC job stamps the manifests with `scripts/stamp-dev-version.mjs` (dev version plus the plugins' exact pins on the library's same-commit dev version) and publishes the library before the plugins. The plugin core and the configurator have no dev release; they ship inlined in the plugin bundles. A plugin whose npm trusted publisher does not exist yet fails its publish step on every push until it is bootstrapped (see the release guide),
