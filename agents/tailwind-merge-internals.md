...
- `.github/workflows/npm-publish.yml`:
    - publishes `dev` tag builds of tailwind-merge and the vite package on `main` pushes: one build job builds both (library first, topologically) and runs both `test:exports` gates, one OIDC job stamps both manifests with `scripts/stamp-dev-version.mjs` (dev version plus the vite package's exact pin on the library's same-commit dev version) and publishes the library before the vite package. The plugin core and the configurator have no dev release; they ship inlined in the plugin bundles. The Next.js package is not in the dev jobs until its npm trusted publisher exists, since a publish step without one fails on every push (bootstrap in the release guide),
