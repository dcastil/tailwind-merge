...
import { createTailwindMerge, getDefaultConfig, twMerge as twMergeCached } from '../src'

import testData from './perf-test-data.json'

for (let [twMerge, cached] of [
    [twMergeCached, true],
    [createTailwindMerge(() => ({ ...getDefaultConfig(), cacheSize: 0 })), false],
] as const) {
    describe('twMerge' + (cached ? ' (cached)' : ''), () => {
        bench('simple', () => {
