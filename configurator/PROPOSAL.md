...
### 11.7 Open decisions before building

1. **Names.** Package: `@tailwind-merge/vite` mirrors `@tailwindcss/vite` and reads as official — pending the npm org reservation; fallback `tailwind-merge-vite`. Plugin factory: `tailwindMerge()`. Subpath: `/tw-merge` (greps like the function) vs `/runtime` vs `/merge`. A future second bundler plugin would promote the subpath into the deferred `@tailwind-merge/runtime` package (§7's revisit condition — a bundler sibling is exactly the "second out-of-repo consumer").
2. **Single-install shape** (§11.3): confirm tailwind-merge-as-dependency, and how loud the dual-copy migration hint should be.
3. **Fallback loudness**: when the plugin isn't active (Jest, misconfigured Vitest project), the subpath silently serves default-config `twMerge` — silent-but-documented vs a dev-only warn-once. Recommendation: silent to start; a warning has no reliable "am I in a test" signal and would fire in every Jest run.
4. **Reload semantics**: full reload on config change (recommended, matches Tailwind) vs virtual-module HMR.
5. **Vite floor**: match `^5.2 || ^6 || ^7 || ^8` with progressive enhancement (recommended — the design needs no 6-only API) vs starting 6+.
6. **Repo layout**: new sideways workspace package (`vite-plugin/`, consistent with `configurator/`) depending on the configurator via `workspace:*`, vs a subpath export of the configurator package. Separate package recommended — the plugin's dependency set (vite peer) shouldn't touch the CLI package, and the stable/unstable boundary stays physical. Workspace note: adding a `vite` devDependency must clear `minimumReleaseAge` (3 days), and Vitest 4 itself runs on Vite — keep versions compatible.
